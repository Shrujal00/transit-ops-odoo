...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        drivers_data = [
            ...
            {'name': 'Richard Jackson', 'email': 'richard.jackson@transitops.com', 'license': 'DL-993810-J', 'expiry_offset': 50, 'status': 'Available'},
        ]
        ...
        # Trips correspond to On Trip vehicles CA-9874-C (idx 2) and NV-2311-G (idx 6)
        # and drivers Carlos Ruiz (idx 2) and James Wilson (idx 5)
        trips_data = [
            {'src': 'Chicago, IL', 'dest': 'Dallas, TX', 'vehicle': vehicles[2], 'driver': drivers[2], 'cargo': 12000, 'rev': 3200.00, 'status': 'Ongoing', 'sched': today},
            {'src': 'Las Vegas, NV', 'dest': 'San Francisco, CA', 'vehicle': vehicles[6], 'driver': drivers[5], 'cargo': 15000, 'rev': 4500.00, 'status': 'Ongoing', 'sched': today},
            {'src': 'Houston, TX', 'dest': 'New Orleans, LA', 'vehicle': vehicles[0], 'driver': drivers[0], 'cargo': 2800, 'rev': 1200.00, 'status': 'Completed', 'sched': today - datetime.timedelta(days=3), 'end': timezone.now() - datetime.timedelta(days=2)},
            {'src': 'New York, NY', 'dest': 'Boston, MA', 'vehicle': vehicles[1], 'driver': drivers[1], 'cargo': 2000, 'rev': 900.00, 'status': 'Completed', 'sched': today - datetime.timedelta(days=5), 'end': timezone.now() - datetime.timedelta(days=4)},
            {'src': 'Seattle, WA', 'dest': 'Portland, OR', 'vehicle': vehicles[4], 'driver': drivers[4], 'cargo': 1000, 'rev': 500.00, 'status': 'Draft', 'sched': today + datetime.timedelta(days=2)},
        ]
